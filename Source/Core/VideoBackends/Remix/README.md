...
`[Settings]`. All 83 knobs are declared in
...
- **Changes take effect at backend init**, i.e. when emulation starts. Restart
  the game after editing. The exceptions are nineteen knobs that are re-read at
  ...
  `RemixUiStrict`, `RemixUiDepth`, `RemixUiAdditiveLightCoverage`,
  `RemixUiWorldView`, `RemixUiOverlayScale`, `RemixUiOverlayCap` and
  `RemixUiFrameCache` (the overlay surface is re-derived from the live window
  every frame, so resizes apply immediately too).
...
| `RemixRestartOnStop` | bool | `True` | Relaunch Dolphin when emulation ends, so the next game starts in a fresh process. The runtime initializes crash reporting, input hooks, the overlay, config layers and per-game folders once per process for the FIRST game it sees; both in-place reset strategies fell short (unloading crashed on threads still inside the module; resident re-resolve kept surfacing subsystems that assume a fresh process — wrong config folders, dead overlay). A fresh process is correct by construction. Double-clicking another game while one runs still works — the queued game rides to the new instance on its command line. NAND/IPL/DFF boots stay in-process (no file path to carry). Remix backend only; never fires from a stop with no preceding session. |
...
| Key | Type | Default | Meaning |
|---|---|---|---|
| `RemixSkyAutoDetect` | int | `1` | `0` off, `1` classify (and push/light per the knobs below), `2` classify *and tag*. **Leave at `1` to render the game's sky.** Mode `2` tags SKY/IGNORE, and on the API path both resolve to *hidden* — see the warning below. Note that classified sky is excluded from the camera vote from mode `1` upward, not only at `2`: a skybox votes for the camera's rotation delta with the translation missing, which is an actively wrong hypothesis rather than a useless one, and mode `1` is not observational anyway — `RemixSkyAtInfinity` defaults on and already moves that geometry. Setting `0` is what turns the exclusion off, because it turns classification off. |
...
| `RemixGxRampAlbedoSkip` | `True` | Take the albedo from a later stage when stage 0's coordinate is generated from a **lit colour channel**. Such a coordinate indexes a toon *ramp*, not the surface, and only one texture reaches Remix — so handing it the ramp paints the model in the ramp. Shading is the path tracer's job. Wind Waker. Falls back to stage 0 when no better stage exists: a canned texture beats an untextured draw. |
| `RemixGxEnvMapAlbedoSkip` | `True` | Take the albedo from a later stage when stage 0's coordinate is generated from the **vertex normal**, which is how an environment map is addressed. Measured on Skyward Sword: 6060 of 7529 traced draws (80.5%) wore a 32×32 reflection while their real texture, up to 256×256, sat unused on a later stage; median albedo went 1024 → 16384 texels. It presented as "all world geometry is flat solid colour". Dropping the reflection is doubly right — a path tracer supplies a real one. Same stage-0 fallback as the ramp rule. |
| `RemixGxUnusedStageAlbedoSkip` | `True` | Take the albedo from the first later stage that both samples a texture **and references it** in its combiner. The two rules above infer a texture's purpose from its *coordinate*; this one asks the combiner directly, because a stage can be texture-enabled and never mention `TEXC`/`TEXA`. Super Mario Galaxy's characters are exactly that: stage 0 is `lerp(ZERO, C0, RAS) + C0` with no `TEX` term, while the texture is used on stage 2 — so Mario arrived wearing a 64×64 mean-243/243/243 mask and rendered nearly colourless. Alpha-only use deliberately does not count: a stage sampling purely for `TexAlpha` is using the texture as a mask, and a mask is not the surface. Ordered last, so the two narrower rules keep first refusal. |
| `RemixGxLitChannelTexGen` | `True` | Run the console's per-vertex lighting when a texture coordinate is generated from a lit colour channel (ported from the software renderer's `TransformUnit.cpp`). Such a coordinate is an **index** into a ramp, not a colour, so substituting the raw vertex colour collapses every vertex onto one texel and the surface renders flat. Isolated A/B on Wind Waker: characters flat yellow before, correct red and dark grey after. |
| `RemixGxEfbAlphaPasses` | `True` | Handle the two-pass EFB alpha-mask idiom — the colour-less pass that primes EFB alpha, then the pass that reads it. Fixes Wind Waker's eyes, eyebrows and a black bar across the face. The pending mask is held as an owned pixel **copy** rather than a borrowed texture pointer: producer and consumer are different draws and the texture cache can evict in between. |
| `RemixGxPreserveHandedness` | `False` | Keep a **mirrored** view basis mirrored when it is re-orthonormalized, instead of forcing right-handed. A cross product silently flips a left-handed basis, turning a rotation of +θ into −θ; The Force Unleashed's median light drift falls from 89/123° to 5.0/0.8° with this on. Cannot fire on a right-handed basis at all — the guard is `dot(z, z0) < 0`, and there the cross product already agrees with row 2 — so it is inert on every other title by construction. Default off regardless: one measured title is not a mandate to change what every other title does. |
...
| `RemixGxAlphaMask` | `True` | When the TEV **alpha** chain reads TEXA through a *different* texmap than the albedo (RE4: compressed colour art on stage 0, a separate intensity mask on a later stage), compose "albedo RGB + mask alpha" into a derived texture and point the material at it. Without this the runtime tests the albedo's own flat-255 alpha and every such coverage card (foliage, fences, hair) renders solid. Alpha-from-albedo draws resolve exactly as before. Counter: `mask`; one `composed alpha-mask texture` log line per pair. |
...
| `RemixViewportRefXfb` | bool | `True` | Gate on which draw may **latch the reference**: only one whose viewport covers >50% of the region the previous frame's XFB copy presented. First-draw-wins breaks any game that renders an off-screen helper pass before its main scene — Sonic Unleashed draws a 320×240 quarter-rect pass first, which latched as "the screen" and folded the real 640×480 scene out of the frustum, leaving the top-left quarter magnified 2×. The XFB rect, not "the biggest viewport", is the ground truth: a game that renders 512×448 in a corner and presents exactly that rect keeps its sub-rect reference correctly. Stands down (= old behaviour, bit for bit) when no viewport covered the presented rect last frame — split screen, menus, boot. Games whose first draw already covers the presented region latch identically either way. Counter: `ref-deferred`. |
...
| `RemixUiTagRouting` | bool | `True` | Route each 2D draw by the Remix dev menu's texture category: **UI Texture** composites it and bypasses every drop heuristic, **Ignore** discards it, **World Space UI** sends it through the mode-`2` world plane. To click-tag elements — the only way to reach an untextured white box, since it has no grid thumbnail — flip `RemixUiWorldView` on, which routes the whole 2D layer world-side. Textured 2D draws are registered into the runtime's texture grid so they *do* get a thumbnail. Inert until something is tagged, so on by default. Live. Tags are polled, so they apply ~2 frames late. Counters: `ui-tags ui/ign/world/tagmode/persp/wref/pskin`, `tex-reg`, `tagsets`, `uistate`. |
| `RemixUiTagPerspective` | bool | `True` | Extend the **UI Texture** tag to draws the game submits with a *perspective* projection. Not every HUD is 2D: Resident Evil 4 parks its health ring, ammo counter and nameplates a short way in front of the camera and draws them through the ordinary 3D frustum, so tagging them did nothing at all (measured: seven UI tags, zero matching draws). With this on, a tagged 3D draw is diverted into the same screen overlay the 2D layer uses, mapped through the game's own perspective matrix. Tagging works exactly as it does in 2D — per texture, or per mesh identity for an untextured draw. **Only the UI tag acts here**: `Ignore` and `World Space UI` on a 3D draw are already applied by Remix itself, which sees a real draw call for them, so this side deliberately leaves them alone. `RemixUiStrict` also stays 2D-only — applied to untagged 3D draws it would delete the world. While `RemixUiWorldView` is on a tagged element returns to the world so it stays clickable and the tag can be removed. **Caveat: the overlay has no depth against the *world*.** A tagged element the game relied on being occluded by world geometry will draw over everything; the remedy is to remove the tag. (Depth *among* overlay draws is honoured — see `RemixUiDepth`.) Subordinate to `RemixUiTagRouting` — the divert requires both. Live, which is what makes "HUD or world geometry?" an A/B rather than a rebuild. Counters: `persp` (draws diverted), `wref` (handed back to the world for straddling the eye plane or spanning too much depth — expect 0; a non-zero here means the tag is on something that is not actually screen-parked), `pskin` (kept world-side because the draw is GPU-skinned). |
...
| `RemixUiWorldView` | bool | `False` | The click-to-tag view: show the game's whole 2D layer as real, clickable world geometry instead of compositing it. Flip on to tag (including untextured white boxes, which no grid thumbnail can reach), off to play — it is live, so no restart. This used to happen automatically whenever the dev menu was open, which yanked the HUD into the world every time the menu was opened for any other reason. Counter: `tagmode`. |
| `RemixUiDepth` | bool | `True` | Honour each **2D** overlay draw's real GX z mode with a depth plane. The overlay is otherwise a pure painter's algorithm — later draws paint over earlier — which many 2D HUDs are fine with, but not all: Super Monkey Ball layers its title UI across graduated depths with the z test on, and order alone gets it wrong. Depth-tested draws interpolate the console's own screen z (same viewport mapping as the software renderer) and test/write after the alpha test, exactly like hardware late-z. Draws with the test off never touch the plane, so `False` is the old painter's algorithm byte for byte. Diverted **perspective** draws never participate: their z is meaningless through the 2D depth mapping (measured reading −11 million in SMB — it clamps to nearest-possible and locks every 2D draw out of the pixel), so they composite in submission order as before. Live. Counter: `ztest` (in the `UI (...)` group) — non-zero says the plane is engaged. |
| `RemixUiAdditiveLightCoverage` | bool | `True` | Let an **additive** 2D draw hide the traced scene only as much as it brightens it. The overlay is composited straight-alpha, so its alpha channel is *coverage* — how much of the traced pixel this layer replaces — and additive blending has no coverage of its own: on console the draw adds `src_rgb * src_alpha` and hides nothing, so a texel adding black is invisible however opaque its alpha claims to be. Charging coverage to that alpha paints it solid black instead. Skyward Sword's bloom composite is a full-screen `SRC_ALPHA/One` quad resolving TEV alpha to 1.0 over a mostly black texture, and it rendered the game as a black screen with only the HUD visible; neither existing filter can reach it (the pre-world rule needs a frame with no world geometry, this arrives after ~980 world draws; the full-screen opaque rule needs blending off or One/Zero). Coverage becomes the brightest channel the draw contributes, so black takes none and a white flash takes all of it and reads exactly as before; dim glows stop punching near-black holes. Note a full-screen additive pass still cannot be represented *faithfully* in one straight-alpha layer — the remaining attenuation is `(1 - coverage)` and is inherent to the compositing model, not to this rule. Live, and part of the frame cache's key. |
...
| `RemixEfbDropAuxPass` | bool | `True` | Drop perspective draws that only existed to **feed** a discarded copy: their viewport rect was copied to a non-XFB destination **with clear** last frame and the copy was discarded. On console the clear erased those pixels and the main pass overdrew the region; as path-traced world geometry nothing would, so (after `RemixViewportRefXfb`) they would linger as a ghost mini-scene in a corner of the view. Whole-rect match with 1px slop — never containment — and never a viewport covering >50% of the presented region; stands down whenever the reference gate does. A dropped draw still counts into the Scene copy signal, or the helper pass's own copy would reclassify Composed2D and execute. Counter: `aux` in the frame line. |
