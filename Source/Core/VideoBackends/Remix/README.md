...
| `RemixUiDepth` | bool | `True` | Honour each overlay draw's real GX z mode with a depth plane. The overlay is otherwise a pure painter's algorithm — later draws paint over earlier — which is all any ordinary 2D HUD needs, because those draw with the z test off. A HUD drawn through the 3D frustum is world geometry and may rely on depth instead: Resident Evil 4 submits its HUD background *after* its text, parked *behind* it, and lets the z test sort them — order alone puts the background on top. Depth-tested draws interpolate the console's own screen z (same viewport mapping as the software renderer) and test/write after the alpha test, exactly like hardware late-z. Draws with the test off never touch the plane, so `False` is the old painter's algorithm byte for byte. Live. Counter: `ztest` (in the `UI (...)` group) — non-zero says the plane is engaged. |
